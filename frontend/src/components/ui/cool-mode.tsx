...
const applyParticleEffect = (
  ...
  const limit = 45
  // Мгновенный залп по клику — иначе частицы летят только пока кнопка зажата
  const burstCount = options?.particleCount ?? 10
  ...
  const tapHandler = (e: MouseEvent | TouchEvent) => {
    ...
    // Залп сразу при нажатии — эффект виден и на коротком клике
    for (let i = 0; i < burstCount && particles.length < limit; i++) {
      generateParticle()
    }
  }
