...
if ([string]::IsNullOrWhiteSpace($iscc)) {
    ...
    try {
        ...
        Write-WarnMsg "Use Install-KonturMarkirovka.ps1 from dist\installer instead."
        exit 0
    }
}

if ([string]::IsNullOrWhiteSpace($iscc)) {
    Write-WarnMsg "ISCC.exe still not found. Payload zip is ready; install Inno Setup 6 to build Setup.exe."
    exit 0
}
