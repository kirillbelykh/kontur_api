...
class BarTenderPrintTests(unittest.TestCase):
    def test_bind_format_to_selected_printer_sets_printer_name(self):
        ...
        bartender_print._bind_format_to_selected_printer(fake_format, "Printer 2")
    ...
    def test_run_sdk_print_falls_back_to_command_line_when_com_print_fails(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            template_path = temp_root / "template.btw"
            csv_path = temp_root / "codes.csv"
            ...
            with (
                ...
                mock.patch.object(
                    bartender_print,
                    "_run_com_print",
                    side_effect=bartender_print.BarTenderPrintError("COM failed"),
                ) as com_print_mock,
                mock.patch.object(
                    bartender_print,
    ...
    def test_build_bartender_cli_data_file_adds_field_headers(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            prepared_csv_path, record_count = bartender_print._build_bartender_cli_data_file(csv_path)
